...
exports.getLeaderships = async (req, res) => {
  try {
    const data = await Leadership.find()
      .populate("trainingType", "topic") // ✅ Add this line
      .populate({
        path: "employee",
        populate: {
          path: "userAccount",
...
exports.createLeadership = async (req, res) => {
  try {
    const data = {
      ...
      // 🚩 AUDIT: Standardized to req.user.username to match other modules
    ...
    // Deep populate after save for the frontend response
  ...
  try {
    ...
    if (!authorizedRoles.includes(req.user.role)) {
      return res.status(403).json({ 
        success: false, 
        message: "Forbidden: Leadership role required to delete records." 
      });
    }
