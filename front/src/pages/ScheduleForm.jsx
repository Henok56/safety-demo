  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      if (Array.isArray(raw)) {
        const list = raw
          .map(u => {
            console.log(`📝 Mapping user:`, u.firstname, u.lastname, u._id);
            return {
          ...
          .filter(emp => {
            const isValid = emp.label && emp.label !== " ";
            console.log(`✅ Filter check - ${emp.label}: ${isValid}`);
            return isValid;
          })
        ...
        console.error("❌ raw is not an array:", raw);
        setEmployees([]);
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      tasks.map((t, i) => ({
        "#": i + 1,
        Activity: t.activity,
        "Assigned To": t.assignedTo && typeof t.assignedTo === 'object' 
          ...
          <div className="form-section">
            ...
            <div className="form-row">
              ...
              <div className="form-group flex-1">
                <label>Assigned To * ({employees.length} employees)</label>
